...
class KokoroTTS:
    ...
    def __init__(self):
        ...
        if not SharedModels.models_loaded:
            logger.error("Models not loaded! Call load_models() before creating KokoroTTS")
            raise RuntimeError("Shared models not loaded")

        self.pipeline = SharedModels.kokoro_pipeline
        self.device = SharedModels.kokoro_device
        logger.debug(f"KokoroTTS using shared pipeline on {self.device}")
